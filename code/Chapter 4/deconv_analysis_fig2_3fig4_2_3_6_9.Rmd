---
title: "Cell deconvolution results analysis/modelling; Figures 2.3, 4.2, 4.3, 4.6 & 4.9"
...
# load libraries and package with custom made changes
library(ggrepel)
...
```{r, fig4_2, fig.width= 15, fig.height=8, warning = FALSE}
# load data; here we use elife dataset with all samples
...
# test hepatocyte proportion differences
stat.test <- merged %>%
...
) + scale_y_continuous(breaks = c(0, 0.1, 0.25, 0.5, 0.75, 0.9)) + ylab("Hepatocyte proportion") + scale_color_manual(values = cols, name = "condition") + scale_x_discrete(labels = c("Roskams-Hieter et al. (2022)", "Chen et al. (2022)", "Zhu et al. (2021)", "Block et al. (2022)")) + theme_pubclean(base_size = 24, base_family = "Helvetica") + guides(colour = guide_legend(nrow = 1)) + theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 20), plot.caption = element_text(size = 14, hjust = 1, vjust = 0, face = "italic", color = "black"))
stat.test <- stat.test %>%
  add_xy_position(x = "dataset", dodge = 0.8)
p0 <- bxp + stat_pvalue_manual(stat.test, label = "{p.adj.signif}", size = 7, family = "Helvetica")

# export plot
ggsave(plot = p0, file = "./figures/fig4_2.png", units = "mm", device = ragg::agg_png, height = 300, width = 500, scaling = 0.9, limitsize = FALSE, dpi = 400, bg = "transparent")

# export the test results
hepato_stat <- stat.test
hepato_effect <- stat.test_effect
save(hepato_stat, hepato_effect, file = "./data/hepato_test.RData")
```

# Hepatocyte proportion based binarization; Figure 4.3

```{r, hep_bi, fig.width= 15, fig.height=8, warning = FALSE}
# load the results; elife dataset only with HD and LC samples
...
# determine optimal hepatocyte proportion cut-off
acc_vec <- c()
sense_vec <- c()
spec_vec <- c()
...
z1 <- data.frame(prop = c(20:40) / 100, acc = acc_vec, sense = sense_vec, spec = spec_vec) %>%
  ...
  geom_point(size = 3) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10), expand = c(0.01, 0.01), name = "Hepatocyte proportion cutoff") +
  ...
  geom_vline(xintercept = 0.27, linetype = "dashed") +
  ggplot2::theme_minimal(base_size = 18, base_family = "Helvetica") +
  theme(plot.title = element_text(hjust = 0.5, vjust = 2.5, face = "bold", size = 20), plot.subtitle = element_text(hjust = 0.5, vjust = 2.5, face = "bold"), axis.text.x = element_text(angle = 30, family = "Helvetica", hjust = 1, vjust = 1), panel.grid.minor = element_blank()) +
  guides(color = guide_legend(override.aes = list(size = 5)))

# 0.27 is the optimal cut-off
...
z2 <- plot_confusion_matrix(conf_mat, counts_on_top = T, add_normalized = F, class_order = c("HD", "LC"), font_counts = cvms::font(size = 9, color = "black"), font_col_percentages = cvms::font(size = 5.5, color = "black"), font_row_percentages = cvms::font(size = 5.5, color = "black"), arrow_size = 0.06, palette = "Blues", intensity_by = "counts", darkness = 0.5) + ggplot2::theme_minimal(base_size = 16, base_family = "Helvetica") + theme(plot.title = element_text(hjust = 0.5, vjust = 2.5, face = "bold", size = 20), plot.subtitle = element_text(hjust = 0.5, vjust = 2.5, face = "bold"))

# merge the plots
z1
z2

# export the predicted condition based on cut-off  results
hepato <- merged
save(hepato, file = "./data/hepato_res.RData")
```

# Logistic regression modelling; Figure 4.6
...
conf_mat <- eval$`Confusion Matrix`[[1]]
p2b <- plot_confusion_matrix(conf_mat, counts_on_top = T, add_normalized = F, class_order = c("HD", "LC"), font_counts = font(size = 9, color = "black"), font_col_percentages = font(size = 5.5, color = "black"), font_row_percentages = font(size = 5.5, color = "black"), arrow_size = 0.06, palette = "Blues", intensity_by = "counts", darkness = 0.5) + ggplot2::theme_minimal(base_size = 16, base_family = "Helvetica") + theme(plot.title = element_text(hjust = 0.5, vjust = 2.5, face = "bold"), plot.subtitle = element_text(hjust = 0.5, vjust = 2.5, face = "bold"))
...
# export the model and the determined best threshold
save(elnet_npj, best_thr, file = "./data/deconv_logmodel.RData")
...
deconv_npj <- data.frame(resp = as.factor(c(npj_clean$cond)), pred_class = c(pd_npj), pred = aggregate(LC ~ rowIndex, elnet_npj_tune$pred, mean)[, "LC"]) %>%
  dplyr::mutate(resp = ifelse(resp == "LC", 1, 0))
save(cfdeconv_log, deconv_npj, file = "./data/cfdeconv_res.RData")
```

# Integrated modeling

```{r, int_model, fig.width= 15, fig.height=8, warning = FALSE}
# load data
load("./data/deconv_res_filt.RData")
# data clenaup
elife_clean <- elife_meta_deconv_filt %>%
...
# load the data needed tor gene marker analysis
...
# npj dataset
select_datac <- which((row.names(npj_batch)) %in% (gene_list))
data_npj <- cbind(npj_meta, t(npj_batch[select_datac, ])) %>%
  dplyr::select(c(gene_list, cond)) %>%
  mutate(cond = factor(cond, levels = c("HD", "LC")))
# elife
select_datac <- which((row.names(elife_countm_filt)) %in% (gene_list))
data_elife <- cbind(elife_meta_filt, t(elife_countm_filt[select_datac, ])) %>%
  dplyr::select(c(gene_list, cond)) %>%
  dplyr::mutate(cond = factor(cond, levels = c("HD", "LC")))
# therano
select_datac <- which((row.names(therano_countm)) %in% (gene_list))
data_therano <- cbind(therano_meta, t(therano_countm[select_datac, ])) %>%
  dplyr::select(c(gene_list, cond)) %>%
  dplyr::mutate(cond = factor(cond, levels = c("HD", "LC")))
# frontiers
select_datac <- which((row.names(frontiers_countm)) %in% (gene_list))
data_frontiers <- cbind(frontiers_meta, t(frontiers_countm[select_datac, ])) %>%
  dplyr::select(c(gene_list, cond)) %>%
  dplyr::mutate(cond = factor(cond, levels = c("HD", "LC")))

# rename the gene name for easier coding
...
# merge the deconv results with gene marker expression data per dataset
...
# optimal hyperparameters for an RF model
set.seed(5)
...
# assign new names to the plots
int1 <- p1
int2 <- p2b

# modify for easier use later
comb_rf <- all_roc %>%
  dplyr::mutate(resp = ifelse(resp == "LC", 1, 0)) %>%
  dplyr::mutate(pred_class = ifelse(pred_class == "LC", 1, 0))

# create dataset for the training phase results
int_roc <- comb_rf
int_npj <- data.frame(resp = as.factor(c(npj_merged$cond)), pred_class = c(pred_npj), pred = model_train$votes[, 2]) %>%
  dplyr::mutate(resp = ifelse(resp == "LC", 1, 0)) %>%
  dplyr::mutate(pred_class = ifelse(pred_class == "LC", 1, 0))
save(int_npj, int_roc, file = "./data/int_res.RData")
```

# create comparison figures; Figure 4.9

```{r, fig4_9, warning = FALSE, fig.width= 15, fig.height=8}
# load marker data
...
## get metrics
merged_filt <- merged %>% dplyr::filter(dataset != "npj") # no npj as it will be part of the training phase
# hepatocye proportion cut-off based classification results
...
# same steps for the 3 Random Forest gene marker models + the integrated model
...
# calculate additional metrics (e.g., sensitivity)
...
## get the lower and upper bounds of the confidence interval for the metrics
...
# create training phase figures; Figure 2.3

```{r, train_figs, warning = FALSE, message = FALSE, fig.width= 15, fig.height=8}
# aggregate the training phase results for all models
...
# export
ggsave(plot = p1, file = "./figures/fig2_3.png", units = "mm", device = ragg::agg_png, height = 90, width = 170, scaling = 0.45, limitsize = FALSE, dpi = 300)
```
