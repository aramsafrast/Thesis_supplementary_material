---
title: "WGCNA of cfRNA samples & generation of Supplementary File 2 - cfRNA Modules"
...
snakemake <- Snakemake(
    ...
    output = list('markdown_results/cf_wgcna_suppl_file2.html'),
    params = list(),
    wildcards = list(),
    threads = 15,
    log = list(),
...
# Exporting module cf-yellow (Cytoscape preparation)
***

```{r cytoscape, message=F, warning=F}
dir.create("./cfrna/results/modules/")
options(stringsAsFactors = FALSE)
# match the genes belonging to the cf-yellow module
probes <- colnames(cfrna_wgcna_input)
modules <- "cf-yellow"
inModule <- is.finite(match(cfmergedcolors, modules))
modProbes <- probes[inModule]
# calculate the connectivity values
IMConn <- softConnectivity(cfrna_wgcna_input[, modProbes], type = "unsigned", power = 6)
# as only 9 genes have connectivity above 50, we will export connections that include those 9 genes
table(IMConn > 50)
nTop <- 9
# get the position of those 9 genes, i.e. arrange from smallest to highest, get the ranks and then get the position of the 9 genes with highest connectivity
top <- (rank(-IMConn) <= nTop)
# get only cf-yellow results
modTOM <- TOM[inModule, inModule]
# export in a Cytoscape ready format
...
# Generation of Supplementary File 2 - cfRNA Modules
***
...
write.xlsx(dataset_names, sheetName = module, file = "./suppl_files/Supplementary File 2 - cfRNA Modules.xlsx")
```
